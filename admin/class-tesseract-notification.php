...
class Tesseract_Notification {
	...
	//is-dismissible
		return '<div class="tesseract-notice notice  ' . esc_attr( $this->options['type'] ) . '" id="' . esc_attr( $this->options['id'] ) . '"' . $this->parse_nonce_attribute() . '>' . wpautop( $this->message ) . '</div>' . PHP_EOL;
	}
	...
	/**
	 * Returns a data attribute containing the notification_id if present
	 *
	 * @return notification id
	 */
	public function get_notification_id() {
		return  $this->options['notification_id'];
	}	
}
